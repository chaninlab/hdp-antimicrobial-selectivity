...
# Getting the Data:
github_url = "Paeruginosa_desc_from_1st_run_master.csv"
...
for seed in range(123, 148):                                              # Iterating the Loop for 25 times.
  ...
  model = RandomForestClassifier(n_jobs=-1, random_state=seed,            # Instantiating the Classifier. Changing the value of seed.
                                 oob_score=True)                          # Returning the OOB score.
  model.fit(data, activity_shuffled)                                                 # Training the Classifier.

  #@ Evaluation Metrics:
  predictions = model.predict(data)                                       # Making the Predictions.
  oob = model.oob_score_                                                  # Calculating the OOB score.
  oob_list.append(oob)
  recall = recall_score(activity_shuffled, predictions)                              # Calculating the Recall score.
  recall_list.append(recall)
  kappa = cohen_kappa_score(activity_shuffled, predictions)                          # Calculating the Kappa score.
  ...
  #@ PERMUTATION Features Importance:
  permutation_important = permutation_importance(model, data, activity_shuffled, n_repeats=5,    # Permutation Importance.
...
scores.to_csv("./Paeruginosa_evaluation_metrics_default.csv")                                  # Saving the DataFrame into csv.
...
gini_importance.to_csv("./Paeruginosa_gini_importance_default.csv")                                    # Saving the DataFrame.
...
permutation_importance_df.to_csv("./Paeruginosa_permutation_importance_default.csv")                             # Saving the DataFrame.
